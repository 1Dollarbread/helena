...
class Repl:
    ...
    async def cmd_voice_setup(self, args: list[str]) -> None:
        stt_status = "ready" if voice.check_available() is None else "needs install (see below)"
...
[bold]Voice input (speech-to-text)[/bold] — {stt_status}
  ...
  1. pip install -e ".[voice]"
  2. /voice — records until you press Enter, transcribes, sends it like typed text
...
[bold]HELENA's spoken voice (text-to-speech)[/bold] — {tts_status}
  ...
  2. export ELEVENLABS_API_KEY=your-key-here   (in your shell profile to persist it)
  3. Optional: pick a voice at https://elevenlabs.io/voice-library, copy its
     voice ID, and export ELEVENLABS_VOICE_ID=that-id — otherwise a stock
     voice ("Rachel") is used by default
  4. /say testing one two three   — to confirm it works
  5. /speak on                     — to have every reply spoken automatically
""")
